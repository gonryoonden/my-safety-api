...
async function searchLaws(lawText) {
  ...
  try {
    ...
    if (items.length) return { items, debug: { source: "xml", ct, url: urlXML.replace(/OC=[^&]+/,"OC=***") } };
...
module.exports = async (req, res) => {
  ...
  try {
    // Vercel Node 함수에서 req.body가 문자열일 수 있으니 보강
    const raw = req.body || {};
    ...
    if (!cand) return res.status(502).json({ status: "ERROR", message: "No law found", debug });
